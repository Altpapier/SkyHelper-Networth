...
const {
  applicationWorth,
  enchantsWorth,
} = require("../constants/applicationWorth");
const {
  blockedEnchants,
  ignoredEnchants,
  stackingEnchants,
...
function starCost(prices, upgrade, star) {
  const upgradePrice = upgrade.essence_type
    ? prices[`essence_${upgrade.essence_type.toLowerCase()}`]
    : prices[upgrade.item_id?.toLowerCase()];
  if (!upgradePrice) return;

  const calculationData = {
    id: upgrade.essence_type
      ? `${upgrade.essence_type}_ESSENCE`
      : upgrade.item_id,
    type: star ? "star" : "prestige",
    price:
      (upgrade.amount || 0) *
      (upgradePrice || 0) *
      (upgrade.essence_type ? applicationWorth.essence : 1),
...
function starCosts(prices, calculation, upgrades, prestigeItem) {
  ...
  if (prestigeItem && datas.length) {
    ...
    if (prestige && prices[prestigeItem.toLowerCase()])
      calculationData.price += prices[prestigeItem.toLowerCase()];
...
const calculateItem = (item, prices, returnItemData) => {
  ...
  if (item?.tag?.ExtraAttributes?.id) {
    ...
    if (ExtraAttributes.skin) {
      if (prices[`${itemId}_skinned_${ExtraAttributes.skin.toLowerCase()}`])
    ...
    if (
      ...
      itemName = `${itemName} (${
        skyblockItem.tier
          ? titleCase(skyblockItem.tier.replaceAll("_", " "))
          : "Unknown"
      })`;
    } else if (itemName.endsWith(" Exp Boost")) {
      itemName = `${itemName} (${
        skyblockItem.id
          ? titleCase(skyblockItem.id.split("_").at(-1))
          : "Unknown"
      })`;
    }
    ...
    // RUNES (Item)
    if (
      ExtraAttributes.id === "RUNE" &&
      ExtraAttributes.runes &&
      Object.keys(ExtraAttributes.runes).length > 0
    ) {
    ...
    if (ExtraAttributes.id === "NEW_YEAR_CAKE")
      itemId = `new_year_cake_${ExtraAttributes.new_years_cake}`;
    // PARTY_HAT_CRAB (Item)
    if (
      ExtraAttributes.id.startsWith("PARTY_HAT_CRAB") &&
      ExtraAttributes.party_hat_color
    )
      itemId = `${ExtraAttributes.id.toLowerCase()}_${
        ExtraAttributes.party_hat_color
      }`;
    // DCTR_SPACE_HELM (Editioned)
    if (
      ExtraAttributes.id === "DCTR_SPACE_HELM" &&
      ExtraAttributes.edition !== undefined
    )
      itemId = "dctr_space_helm_editioned";
    // SHINY
    if (ExtraAttributes.is_shiny && prices[`${itemId}_shiny`])
      itemId = `${itemId}_shiny`;
    ...
    if (
      ExtraAttributes.id == "PICKONIMBUS" &&
      ExtraAttributes.pickonimbus_durability
    ) {
      const reduction =
        ExtraAttributes.pickonimbus_durability / pickonimbusDurability;
    ...
    if (
      ExtraAttributes.price &&
      ExtraAttributes.auction !== undefined &&
      ExtraAttributes.bid !== undefined
    ) {
      const pricePaid =
        Number(ExtraAttributes.price) * applicationWorth.shensAuctionPrice;
      if (pricePaid > price) {
        price = pricePaid;
        calculation.push({
          id: itemId,
    ...
    if (itemId === "midas_staff" || itemId === "midas_sword") {
      ...
      const type =
        itemId === "midas_sword" ? "MIDAS_SWORD_50M" : "MIDAS_STAFF_100M";
    ...
    if (itemId === "enchanted_book" && ExtraAttributes.enchantments) {
      ...
      if (Object.keys(ExtraAttributes.enchantments).length == 1) {
        ...
        calculation.push(calculationData);
        itemName = titleCase(
          (name === "aiming" ? "dragon tracer" : name).replace(/_/g, " ")
        );
      } else {
        ...
        for (const [name, value] of Object.entries(
          ...
          const calculationData = {
            ...
            type: "enchant",
            price:
              (prices[`enchantment_${name.toLowerCase()}_${value}`] || 0) *
              applicationWorth.enchants,
            count: 1,
          };
      ...
      for (let [name, value] of Object.entries(ExtraAttributes.enchantments)) {
        ...
        if (
          ...
          const calculationData = {
            ...
            price:
              (prices["sil_ex"] || 0) *
              (value - (itemId === "stonk_pickaxe" ? 6 : 5)) *
              applicationWorth.silex,
            count: 1,
          };
          price += calculationData.price;
          calculation.push(calculationData);
        ...
        const calculationData = {
          ...
          type: "enchant",
          price:
            (prices[`enchantment_${name}_${value}`] || 0) *
            (enchantsWorth[name] || applicationWorth.enchants),
          count: 1,
        };
    ...
    if (ExtraAttributes.attributes) {
      for (const [attribute, tier] of Object.entries(
        ...
        if (
          attributesBaseCosts[itemId] &&
          prices[attributesBaseCosts[itemId]] < baseAttributePrice
        ) {
          baseAttributePrice = prices[attributesBaseCosts[itemId]];
        } else if (
          /^(|hot_|fiery_|burning_|infernal_)(aurora|crimson|terror|hollow|fervor)_helmet$/.test(
            itemId
          ) &&
          ...
          /^(|hot_|fiery_|burning_|infernal_)(aurora|crimson|terror|hollow|fervor)(_chestplate|_leggings|_boots)$/.test(
            itemId
          )
        ) {
          const kuudraPrices = [
            prices[`kuudra_chestplate_${attribute}`],
            prices[`kuudra_leggings_${attribute}`],
            prices[`kuudra_boots_${attribute}`],
          ].filter((v) => v);
          const kuudraPrice =
            kuudraPrices.reduce((a, b) => a + b, 0) / kuudraPrices.length;
          if (
            kuudraPrice &&
            (!baseAttributePrice || kuudraPrice < baseAttributePrice)
          )
            baseAttributePrice = kuudraPrice;
        }
        if (!baseAttributePrice) continue;
        const attributePrice =
          baseAttributePrice * shards * applicationWorth.attributes;
    ...
    if (ExtraAttributes.wood_singularity_count) {
      const calculationData = {
        ...
        price:
          (prices["wood_singularity"] || 0) *
          ExtraAttributes.wood_singularity_count *
          applicationWorth.woodSingularity,
    ...
    if (ExtraAttributes.tuned_transmission) {
      const calculationData = {
        ...
        price:
          (prices["transmission_tuner"] || 0) *
          ExtraAttributes.tuned_transmission *
          applicationWorth.tunedTransmission,
    ...
    if (ExtraAttributes.mana_disintegrator_count) {
      const calculationData = {
        ...
        price:
          (prices["mana_disintegrator"] || 0) *
          ExtraAttributes.mana_disintegrator_count *
          applicationWorth.manaDisintegrator,
    ...
    if (ExtraAttributes.thunder_charge && itemId === "pulse_ring") {
      const thunderUpgrades = Math.floor(
        ExtraAttributes.thunder_charge / 50_000
      );
      const calculationData = {
        id: "THUNDER_IN_A_BOTTLE",
        type: "thunder_charge",
        price:
          (prices["thunder_in_a_bottle"] || 0) *
          thunderUpgrades *
          applicationWorth.thunderInABottle,
    ...
    if (
      ...
      if (validRunes.includes(runeId)) {
        const calculationData = {
          ...
          type: "rune",
          price:
            (prices[`rune_${runeId}`.toLowerCase()] || 0) *
            applicationWorth.runes,
          count: 1,
        };
        price += calculationData.price;
        calculation.push(calculationData);
    ...
    if (ExtraAttributes.hot_potato_count) {
      if (ExtraAttributes.hot_potato_count > 10) {
        const calculationData = {
          ...
          price:
            (prices["fuming_potato_book"] || 0) *
            (ExtraAttributes.hot_potato_count - 10) *
            applicationWorth.fumingPotatoBook,
      ...
      const calculationData = {
        ...
        price:
          (prices["hot_potato_book"] || 0) *
          Math.min(ExtraAttributes.hot_potato_count, 10) *
          applicationWorth.hotPotatoBook,
    ...
    if (ExtraAttributes.dye_item) {
      const calculationData = {
        ...
        type: "dye",
        price:
          (prices[ExtraAttributes.dye_item.toLowerCase()] || 0) *
          applicationWorth.dye,
        count: 1,
      };
      price += calculationData.price;
      calculation.push(calculationData);
    ...
    if (ExtraAttributes.art_of_war_count) {
      const calculationData = {
        ...
        type: "the_art_of_war",
        price:
          (prices["the_art_of_war"] || 0) *
          ExtraAttributes.art_of_war_count *
          applicationWorth.artOfWar,
    ...
    if (ExtraAttributes.artOfPeaceApplied) {
      const calculationData = {
        ...
        price:
          (prices["the_art_of_peace"] || 0) *
          ExtraAttributes.artOfPeaceApplied *
          applicationWorth.artOfPeace,
    ...
    if (ExtraAttributes.farming_for_dummies_count) {
      const calculationData = {
        ...
        price:
          (prices["farming_for_dummies"] || 0) *
          ExtraAttributes.farming_for_dummies_count *
          applicationWorth.farmingForDummies,
    ...
    if (ExtraAttributes.talisman_enrichment) {
      const enrichmentPrice = enrichments.reduce(
        (acc, val) => Math.min(acc, prices[val.toLowerCase()] || 0),
        Infinity
      );
    ...
    if (ExtraAttributes.rarity_upgrades > 0 && !ExtraAttributes.item_tier) {
      ...
      if (
        ...
        lastLoreLine?.includes("HATCCESSORY")
      ) {
        const recombApplicationWorth =
          itemId === "bone_boomerang"
            ? applicationWorth.recomb * 0.5
            : applicationWorth.recomb;
        const calculationData = {
    ...
    if (ExtraAttributes.gems) {
      ...
      if (skyblockItem?.gemstone_slots) {
        if (ExtraAttributes.gems.formatted) {
          ...
          const ExtraAttributesGems = JSON.parse(
            JSON.stringify(ExtraAttributes.gems)
          );
          skyblockItem?.gemstone_slots.forEach((slot) => {
            if (slot.costs && ExtraAttributesGems.unlocked_slots) {
              for (const [
                index,
                type,
              ] of ExtraAttributesGems.unlocked_slots.entries()) {
            ...
            const key = Object.keys(ExtraAttributesGems).find(
              (k) => k.startsWith(slot.slot_type) && !k.endsWith("_gem")
            );
            if (key) {
              const type = [
                "COMBAT",
                ...
                "MINING",
                "UNIVERSAL",
              ].includes(slot.slot_type)
                ? ExtraAttributesGems[`${key}_gem`]
                : slot.slot_type;
              gems.push({
                type,
                tier:
                  ExtraAttributesGems[key] instanceof Object
                    ? ExtraAttributesGems[key].quality
                    : ExtraAttributesGems[key],
        ...
        if (
          isDivansArmor ||
          /^(|hot_|fiery_|burning_|infernal_)(aurora|crimson|terror|hollow|fervor)(_helmet|_chestplate|_leggings|_boots)$/.test(
            itemId
          )
        ) {
          const application = isDivansArmor
            ? applicationWorth.gemstoneChambers
            : applicationWorth.gemstoneSlots;
          const gemstoneSlots = JSON.parse(
            JSON.stringify(skyblockItem.gemstone_slots)
          );
          for (const unlockedSlot of unlockedSlots) {
            const slot = gemstoneSlots.find(
              (s) => s.slot_type === unlockedSlot
            );
            const slotIndex = gemstoneSlots.findIndex(
              (s) => s.slot_type === unlockedSlot
            );
            if (slotIndex > -1) {
              let total = 0;
              for (const cost of slot.costs || []) {
                if (cost.type === "COINS") total += cost.coins;
                  ...
                  total +=
                    (prices[cost.item_id.toLowerCase()] || 0) * cost.amount;
              }
        ...
        for (const gemstone of gems) {
          const calculationData = {
            ...
            type: "gemstone",
            price:
              (prices[`${gemstone.tier}_${gemstone.type}_gem`.toLowerCase()] ||
                0) * applicationWorth.gemstone,
            count: 1,
          };
          price += calculationData.price;
          calculation.push(calculationData);
      ...
      if (skyblockItem.upgrade_costs.length <= 5) {
        for (const star of Array(starsUsed).keys()) {
          const calculationData = {
            ...
            type: "master_star",
            price:
              (prices[masterStars[star]] || 0) * applicationWorth.masterStar,
            count: 1,
          };
          price += calculationData.price;
          calculation.push(calculationData);
    ...
    if (
      skyblockItem?.upgrade_costs &&
      (dungeonItemLevel > 0 || upgradeLevel > 0)
    ) {
      const level = Math.max(dungeonItemLevel, upgradeLevel);
      price += starCosts(
        prices,
        calculation,
        skyblockItem.upgrade_costs.slice(0, level + 1)
      );
    }
    ...
    if (ExtraAttributes.ability_scroll) {
      for (const id of Object.values(ExtraAttributes.ability_scroll)) {
        const calculationData = {
          ...
          type: "necron_scroll",
          price:
            (prices[id.toLowerCase()] || 0) *
            applicationWorth.necronBladeScroll,
          count: 1,
        };
        price += calculationData.price;
        calculation.push(calculationData);
    ...
    // DRILLS
    const drillPartTypes = [
      "drill_part_upgrade_module",
      "drill_part_fuel_tank",
      "drill_part_engine",
    ];
    if (
      drillPartTypes.some((type) => Object.keys(ExtraAttributes).includes(type))
    ) {
      for (const type of drillPartTypes) {
        if (ExtraAttributes[type]) {
          const calculationData = {
            ...
            type: "drill_part",
            price:
              (prices[ExtraAttributes[type]] || 0) * applicationWorth.drillPart,
            count: 1,
          };
          price += calculationData.price;
          calculation.push(calculationData);
