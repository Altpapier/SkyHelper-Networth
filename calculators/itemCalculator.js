...
const calculateItem = (item, prices, nonCosmetic, returnItemData) => {
    if (item?.tag?.ExtraAttributes?.id === 'PET' && item?.tag?.ExtraAttributes?.petInfo) {
        ...
        return calculatePet(petInfo, prices, nonCosmetic);
    }
    ...
    if (item?.tag?.ExtraAttributes?.id) {
        ...
        if (nonCosmetic) {
            const check = (itemId + itemName).toLowerCase();
            const itemLore = item.tag.display?.Lore || [];
            if (skyblockItem?.category === 'COSMETIC' || check.includes('dye') || check.includes('skin') || itemLore.at(-1)?.includes('COSMETIC')) {
                return null;
            }
        }

        if (ExtraAttributes.skin && !nonCosmetic) {
        ...
        // DYES
        if (ExtraAttributes.dye_item && !nonCosmetic) {
            const calculationData = {
