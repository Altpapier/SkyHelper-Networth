...
const getPetLevelPrices = (pet, prices, nonCosmetic) => {
    ...
    const basePrices = {
        id: `${tierName}${skin && !nonCosmetic ? `_skinned_${skin}` : ''}`,
...
const calculatePet = (pet, prices, nonCosmetic) => {
    const { lvl1, lvl100, lvl200, id } = getPetLevelPrices(pet, prices, nonCosmetic);
    ...
    // SOULBOUND PET SKIN
    if (pet.skin && soulbound && !nonCosmetic) {
